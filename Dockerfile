...
ENTRYPOINT ["/bin/sh", "-c"]
CMD ["/usr/bin/wakunode --relay=true --lightpush=true --filter=true --store=true --websocket-support=true --listen-address=0.0.0.0 --websocket-port=${PORT:-8000} --rln-relay=false"]
